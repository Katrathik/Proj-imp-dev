...
const Manager = () => {
  ...
  // get passwords from db via backend
  ...
  // toggle show and don't show password
  const showPassword = () => {
  ...
  const deletePassword = async (id) => {
    ...
    if(c){
      ...
      let res = await fetch("http://localhost:3000",{method: "DELETE", headers :{
        "Content-type":"application/json"},
      body: JSON.stringify({id})})
      // if we give ...form then all will be deleted , so we give only id JSON.stringify()
    ...
    <>
      ...
      <div className="p-3 md:mycontainer min-h-[88.2vh]">
        ...
        <div className="passwords">
          <h2 className="fond-bold text-2xl py-4">Your Passwords</h2>
          {
            passwordArray.length === 0 && (
              <div> No passwords to show</div>
            ) /* display null if no passwords or else display this entire table as the output in which tr is mapped, measn how much user enters , those many passwords will be seen*/
          }
          {passwordArray.length != 0 && (
